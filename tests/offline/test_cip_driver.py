...
_simple_path = ('192.168.1.100', [PortSegment('bp', 0), ])
_simple_paths = [
    '192.168.1.100',
    '192.168.1.100/0',
    r'192.168.1.100\0',
...
def test_bad_plc_paths(path):
    with pytest.raises((DataError, RequestError), ):
...
def test_get_module_info_returns_expected_identity_dict():
    EXPECTED_DICT = {'vendor': 'Rockwell Automation/Allen-Bradley', 'product_type': 'Programmable Logic Controller',
                     'product_code': 89, 'revision': {'major': 20, 'minor': 19}, 'status': b'`0',
                     'serial': 'c00fa09b', 'product_name': '1769-L23E-QBFC1 LOGIX5323E-QBFC1'}

    RESPONSE_BYTES = (b'o\x00C\x00\x02\x13\x02\x0b\x00\x00\x00\x00_pycomm_\x00\x00\x00\x00\x00\x00\x00\x00\n'
...
def test__forward_close_returns_true_if_response():
        driver = CIPDriver(CONNECT_PATH)
        driver._session = 1
        response = (b"o\x00\x1e\x00\x02\x16\x02\x0b\x00\x00\x00\x00_pycomm_"
